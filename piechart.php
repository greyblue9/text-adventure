...
$colors = array('FF4040', 'F19013', 'E4EB1D','2CEB1D', '1D96EB', '3F1DEB','C21DEB','9D8041','737373','CACACA'); // colors of the slices.
...
if (is_array($label))
{
	for ($i = 0; $i < count($label); $i++) 
	{
		...
		{	
			imagefilledrectangle($img,$width+6,$label_place+2,$width+16,$label_place+13,colorHex($img, $colors[$i % count($colors)]));
			imagerectangle($img,$width+6,$label_place+2,$width+16,$label_place+13,colorHex($img, $text_color));
		}
		...
		if ($data[$i] > 0)
		{
			...
			imagestring($img,'5',$width+26,$label_place,$label_output,colorHex($img, $text_color));
			$label_output = '';
